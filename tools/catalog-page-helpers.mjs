...
export async function writeCatalogPage({ outputPath, slug, title, eyebrow, description, sections, unit = "项", placeholder }) {
  ...
  const sectionHtml = sections.map((section) => {
    const entries = section.entries.map((entry) => {
      ...
      const fields = entry.fields.map((field) => `<div${field.kind ? ` class="entry-field-${escapeHtml(field.kind)}"` : ""}><dt>${escapeHtml(field.label)}</dt><dd>${field.html ?? formatValue(field.value)}</dd></div>`).join("\n");
