...
const filters = [...document.querySelectorAll("#scout-filters .filter")];
const entries = [...document.querySelectorAll(".scout-entry")];
const sections = [...document.querySelectorAll(".scout-section")];
const resultCount = document.querySelector("#result-count");
const emptyState = document.querySelector("#scout-empty");
const clearButton = document.querySelector("#clear-scout");
let category = "全部";
...
function render() {
  ...
  for (const entry of entries) {
    const matchesCategory = category === "全部" || entry.dataset.category === category;
    const matchesQuery = !query || normalize(entry.textContent).includes(query);
    entry.hidden = !(matchesCategory && matchesQuery);
    if (!entry.hidden) visible += 1;
  }
  for (const section of sections) {
    section.hidden = !section.querySelector(".scout-entry:not([hidden])");
  }
